  ...
  public List<Map<String, Object>> queue(Long doctorId) {
    return jdbcTemplate.queryForList("""
        SELECT r.id AS "registrationId",
               ...
               COALESCE(NULLIF(r.visitor_name, ''), p.name) AS "patientName",
               COALESCE(NULLIF(r.visitor_gender, ''), p.gender) AS "gender",
               COALESCE(r.visitor_age, p.age) AS "age",
               COALESCE(r.visitor_id, r.patient_id) AS "visitorId",
               COALESCE(NULLIF(r.visitor_type, ''), 'ACCOUNT') AS "visitorType",
               COALESCE(NULLIF(r.visitor_relationship, ''), '本人') AS "visitorRelationship",
