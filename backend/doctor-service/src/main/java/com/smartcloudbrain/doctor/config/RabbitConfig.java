package com.smartcloudbrain.doctor.config;
...
public class RabbitConfig {
  ...
  Queue notificationDispatchQueue() {
    return new Queue(RabbitTopology.NOTIFICATION_DISPATCH_QUEUE, true, false, false,
        Map.of("x-dead-letter-exchange", RabbitTopology.DEAD_EXCHANGE));
  }

  @Bean
  Queue auditLogQueue() {
    return new Queue(RabbitTopology.AUDIT_LOG_QUEUE, true, false, false,
        Map.of("x-dead-letter-exchange", RabbitTopology.DEAD_EXCHANGE));
  }
  ...
  Binding notificationDispatchBinding(
    ...
    return BindingBuilder.bind(notificationDispatchQueue).to(domainExchange).with(RabbitTopology.ROUTING_NOTIFICATION_DISPATCH);
  }
  ...
  Binding auditLogBinding(
    ...
    return BindingBuilder.bind(auditLogQueue).to(domainExchange).with(RabbitTopology.ROUTING_AUDIT_LOG);
  }
