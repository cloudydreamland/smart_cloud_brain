...
```text
前端 -> Gateway -> 业务服务 -> ai-service -> DifyAiProvider -> Dify Workflow -> DeepSeek 等真实模型
```
...
- `DifyAiProvider` 是默认真实模型接入方式；`MockAiProvider` 仅作为本地兜底和测试实现保留。
...
项目默认通过 `DifyAiProvider` 接入 Dify Workflow：
...
> 项目不强调模型医学准确率，而强调 AI 服务化、Prompt 可配置、结构化输出、知识库约束、医生确认和失败降级。当前大模型 Provider 默认是 Dify，调用链路、数据库落库、权限控制和业务闭环都是真实实现。
