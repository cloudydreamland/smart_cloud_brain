...
| AI 集成 | HTTP API、大语言模型服务、知识图谱服务可选 |
...
backend/
  src/main/java/
    config/              CORS、JWT、Swagger、Web 配置
    controller/          REST API
    service/             业务服务
    service/ai/          AI 能力封装
...
- Service 负责业务编排、事务和 AI 能力调用。
- Repository 负责数据访问。
- AI 能力放入 `service.ai` 包，避免散落到 Controller。
...
| 分诊 AI | `AITriageService` | 智能分诊页 |
| 病历 | `MedicalRecordService`、`AIMedicalRecordService` | 问诊病历页、病历详情 |
| 处方 | `PrescriptionService`、`AIPrescriptionCheckService` | 处方开具页、处方详情 |
