...
const departmentForm = ref({ code: "GENERAL", name: "全科/轻症门诊", description: "校园常见轻症初诊与复诊" });
const doctorForm = ref({ name: "李医生", phone: "13900000002", password: "123456", departmentId: 2, title: "主治医师", specialty: "感冒发热、咽痛、腹泻、皮肤过敏", status: "ENABLED" });
...
async function refresh() {
  ...
  knowledge.value = await api.knowledgeEntries();
}

async function saveDepartment() {
  await run("保存科室", async () => {
    await api.saveDepartment(departmentForm.value);
    await refresh();
  });
}
...
async function saveDoctor() {
  await run("保存医生", async () => {
    await api.saveDoctor(doctorForm.value);
    await refresh();
  });
}
...
async function saveDrug() {
  await run("保存药品", async () => {
    await api.saveDrug(drugForm.value);
    await refresh();
  });
}
...
async function savePrompt() {
  await run("保存 Prompt", async () => {
    await api.savePrompt(promptForm.value);
    await refresh();
  });
}
...
async function saveKnowledge() {
  await run("保存知识库", async () => {
    await api.saveKnowledgeEntry(knowledgeForm.value);
    await refresh();
  });
}
...
<template>
  <main class="app-shell">
    <aside class="rail">
      ...
      <h1>管理端</h1>
      <p>维护科室、医生、药品、Prompt 与轻症知识库，不包含支付、医保或库存。</p>
      ...
      <template v-else>
        <section class="panel">
          ...
          <div class="grid three">
            ...
            <label>科室ID<input v-model.number="doctorForm.departmentId" type="number" /></label>
