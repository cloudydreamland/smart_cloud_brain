...
async function clearImage(section: ImageTextSection) {
  if (!section.image || !(await confirm({
    title: "确认移除图文图片",
    message: "将从当前编辑稿中移除图文区块的图片配置。保存并生效或发布后，患者端对应区块才会更新展示。",
    confirmText: "确认清空图片",
    tone: "danger",
  }))) return;
...
async function clearPrimary(section: ImageTextSection) {
