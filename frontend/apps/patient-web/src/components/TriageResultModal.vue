...
<template>
  <Modal :open="open" title="分诊结果" description="AI 分诊仅用于挂号推荐，最终诊断以医生接诊为准。" @close="$emit('close')">
    <div v-if="result" class="triage-result">
      <StatusTag :status="fieldText(result, 'status')" :tone="statusClass(result.status)" />
